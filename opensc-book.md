...
ほとんどの行政手続きはLinuxデスクトップに対応しておらず、特に使い道が無い事を知りました。
そしてひと通りリバース・エンジニアリングを行った後、住基カードはタンスの肥やしとなってしまうのでした。
...
ICチップに格納されている秘密鍵を読み出すことは出来ないようになっており、ICチップの中で安全に暗号・署名処理を行います。
...
将来的には銀行のオンラインバンキングなど民間システムでの利用も想定されています。
...
これは否認防止、つまりこの証明書でデジタル署名された文書への同意を意味します。
...
pkcs15-cryptはカードでデジタル署名を行うためのコマンドラインツールです。
...
![macOSパスワード認証画面](static/macos-login1.png)

カードを読み込むと…

![macOSICカード認証](static/macos-login2.png)
...
![macOSスクリーンロック](static/macos-screenlock.png)
