...
ひと通りリバース・エンジニアリングを行った後、特に使い道が無い事を知りました。
そして住基カードはタンスの肥やしとなってしまいました。
...
この証明書の署名対象はNONCEなどの乱数であって同意したい文書では無いからです。
...
役場でマイナンバーカードを受け取った時、2種類または4種類の暗証番号を設定したはずです。それぞれ以下の通りです。
...
![macOSログイン画面](static/macos-login1.png)

カードを読み込むと…

![ICカードログイン](static/macos-login2.png)
