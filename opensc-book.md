...
サービス開始時は行政手続きをオンラインで行うための用途に限られていました。

公的個人認証自体は住基カードの頃から利用されていましたが、マイナンバーカードからは用途が拡大され、民間サービスでも利用できるようになりました。

また、マイナンバーカードにはデジタル署名用の証明書に加え、ユーザー認証用の証明書が追加されています。
デジタル署名用証明書は契約書などの文書へのデジタル署名、ユーザー認証用証明書はWebサイトにログインする用途が想定されています。
...
このツールを使って公的個人認証APによるデジタル署名を試してみましょう。
...
今回は、小さいテキストファイルに対して署名を行いましたが、対象ファイルが256byte(2048bit)を超えると署名できません。
ですので通常、デジタル署名はメッセージのダイジェストに対して署名を行います。
...
![SSHプロトコル](static/ssh-protocol.pdf){width=100%}
